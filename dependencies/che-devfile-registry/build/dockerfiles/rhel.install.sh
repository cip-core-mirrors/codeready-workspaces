...
yum install -y findutils bash wget yum gzip tar python3-six python3-pip && yum -y clean all
yum install -y skopeo jq && yum update -y skopeo containers-common jq oniguruma && yum -y clean all
